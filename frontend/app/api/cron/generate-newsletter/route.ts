...
export async function GET(req: Request) {
  const query = `
    query {
      hotMarkets: markets(first: 3, orderBy: totalVolume, orderDirection: desc, where: { resolved: false }) {
        question
        totalVolume
        id
      }
      coldMarkets: markets(first: 3, orderBy: totalVolume, orderDirection: asc, where: { resolved: false, totalVolume_gt: "1000000000000000000" }) {
        question
        totalVolume
        id
      }
      topUsers: users(first: 3, orderBy: totalVolume, orderDirection: desc) {
        id
        totalVolume
      }
    }
  ...
  try {
    ...
    const hotMarkets = json.data?.hotMarkets || [];
    const coldMarkets = json.data?.coldMarkets || [];
    const topUsersRaw = json.data?.topUsers || [];
    ...
    if (profiles) {
        profiles.forEach((p: any) => {
            if (p.address && p.username) {
                usernameMap.set(p.address.toLowerCase(), p.username);
            }
        });
    }
        ...
        <body style="margin: 0; padding: 0; font-family: sans-serif; background-color: #0f172a; color: #ffffff;">
            ...
            ${hotMarkets.length > 0 ? hotMarkets.map((m: any) => `
                <div style="background-color: #1e293b; padding: 15px; margin-bottom: 12px; border-radius: 8px; border: 1px solid #334155;">
                    <div style="font-weight: bold; font-size: 16px; margin-bottom: 4px; color: #e2e8f0;">${m.question}</div>
                    <div style="color: #10b981; font-size: 14px; font-weight: bold;">Vol: $${Number(formatEther(BigInt(m.totalVolume))).toLocaleString(undefined, {maximumFractionDigits: 0})}</div>
            ...
            ${coldMarkets.length > 0 ? coldMarkets.map((m: any) => `
                <div style="background-color: #1e293b; padding: 15px; margin-bottom: 12px; border-radius: 8px; border: 1px solid #334155; opacity: 0.8;">
                    <div style="font-weight: bold; font-size: 16px; margin-bottom: 4px; color: #cbd5e1;">${m.question}</div>
                    <div style="color: #94a3b8; font-size: 14px;">Vol: $${Number(formatEther(BigInt(m.totalVolume))).toLocaleString(undefined, {maximumFractionDigits: 0})}</div>
                    <div style="margin-top: 8px;">
            ...
            <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px; background: #1e293b; border-radius: 8px; overflow: hidden;">
                ...
                <tr style="border-bottom: 1px solid #334155;">
                    <td style="padding: 12px 15px; color: #fbbf24; font-weight: bold; width: 30px;">#${index + 1}</td>
                    <td style="padding: 12px 0; font-family: sans-serif; color: #e2e8f0; font-weight: bold;">
                        ${formatName(u.id)}
                    </td>
                    <td style="padding: 12px 15px; text-align: right; color: #10b981; font-weight: bold;">$${Number(formatEther(BigInt(u.totalVolume))).toLocaleString(undefined, {maximumFractionDigits: 0})}</td>
                </tr>
    ...
    console.error(e);
    return new NextResponse('Error generating newsletter', { status: 500 });
  }
}