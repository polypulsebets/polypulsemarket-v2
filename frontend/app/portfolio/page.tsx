...
export default function PortfolioPage() {
  ...
  const handleRedeem = (marketAddress: string) => {
            ...
            <div className="max-w-5xl mx-auto w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="grid md:grid-cols-2 gap-6 mb-10">
                    <div className="bg-gradient-to-br from-slate-900 to-slate-950 p-8 rounded-3xl border border-slate-800 relative overflow-hidden group shadow-xl">
                        ...
                        <div className="text-5xl font-mono font-bold text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-teal-300">${formattedBalance.toLocaleString(undefined, {minimumFractionDigits: 2})}</div>
                        <button onClick={() => writeMint({ address: MOCK_USDT_ADDRESS as `0x${string}`, abi: ERC20_ABI, functionName: 'mint', args: [address as `0x${string}`, parseEther('1000')] })} className="mt-6 text-xs bg-slate-800 text-slate-300 px-4 py-2 rounded-lg hover:bg-emerald-900/30 hover:text-emerald-400 hover:border-emerald-500/50 border border-slate-700 transition-all font-bold flex items-center gap-2"><span>+</span> Mint $1,000 Testnet USDC</button>
                    </div>
                        ...
                        {paginatedMarkets.length > 0 ? (
                            paginatedMarkets.map(market => ( 
                                <PortfolioRow 
                                    ...
                                    // 3. UPDATED: Navigate to separate market page
